...
describe('shouldScrollToNewest', () => {
  it('lands on the newest message when the thread opens', () => {
    ...
    // just asked.
    expect(
      shouldScrollToNewest({ firstRun: true, landOnOpen: true, distanceFromBottomPx: 9999 }),
    ).toBe(true);
  });
  ...
  it('does NOT land on open when she is looking at a widened window', () => {
    ...
    expect(
      shouldScrollToNewest({ firstRun: true, landOnOpen: false, distanceFromBottomPx: 0 }),
    ).toBe(false);
  });

  it('follows a new message when she is already at the bottom', () => {
    expect(
      shouldScrollToNewest({ firstRun: false, landOnOpen: true, distanceFromBottomPx: 0 }),
    ).toBe(true);
  });

  it('tolerates being roughly one bubble short of the bottom', () => {
    expect(
      shouldScrollToNewest({ firstRun: false, landOnOpen: true, distanceFromBottomPx: NEAR_BOTTOM_PX }),
    ).toBe(true);
  });
  ...
  it('leaves her alone when she is scrolled back through history', () => {
    ...
    expect(
      shouldScrollToNewest({ firstRun: false, landOnOpen: true, distanceFromBottomPx: NEAR_BOTTOM_PX + 1 }),
    ).toBe(false);
    expect(
      shouldScrollToNewest({ firstRun: false, landOnOpen: true, distanceFromBottomPx: 4000 }),
    ).toBe(false);
  });
  ...
  it('ignores landOnOpen after the first run', () => {
    ...
    // bottom of it.
    expect(
      shouldScrollToNewest({ firstRun: false, landOnOpen: false, distanceFromBottomPx: 0 }),
    ).toBe(true);
    expect(
      shouldScrollToNewest({ firstRun: false, landOnOpen: false, distanceFromBottomPx: 4000 }),
    ).toBe(false);
  });
