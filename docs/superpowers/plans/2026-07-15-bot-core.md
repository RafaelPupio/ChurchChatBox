...
- **Node 20+.**
- **`drizzle-orm` must be ≥ 0.45.2.** Versions below it carry GHSA-gpj5-g38j-94v9 — SQL injection via improperly escaped SQL identifiers. Every query in this project runs through this ORM.
...
export function route(input: RouterInput): RouterResult {
  ...
  if (message.kind === 'list_reply') {
    ...
    // Canonical digits only. Bare Number() would coerce '+1', '0x1' and '1e0'
    // to 1 and silently select the first item.
    if (/^\d+$/.test(text)) {
...
```ts
    if (mode === 'awaiting_prayer') {
      ...
      // An empty capture would be thanked but never saved (the webhook's save is
      // guarded by a truthy check) — the member would be told their prayer was
      // received while it silently vanished. Re-prompt instead.
      if (prayer === '') {
        return {
...
// Church and menu are each independently idempotent. The neon-http driver has no
// transaction support, so the two inserts cannot be atomic: if the menu insert
// fails, the church row survives with zero menu rows. A guard that only checked
// for the church would then report "already seeded" forever and never repair it,
// leaving members staring at an empty menu. Checking each separately means a
...
async function seed() {
  ...
  if (menuItems.length === 0) {
    ...
    // Never clobber staff edits: they may have reworded, reordered, or hidden items.
...
Expected:
```
Created church: Minha Igreja (id: <uuid>)
Inserted 9 menu items
```

- [ ] **Step 3: Verify idempotency and recovery**

Run: `npm run db:seed`
Expected: `Using existing church: Minha Igreja` + `Church already has 9 menu items — no changes made`.

Then prove recovery from a partial failure: delete the menu rows (`DELETE FROM menu_item;`) and rerun.
Expected: the church is reused and the 9 rows are reinserted — the seed repairs itself rather than reporting "already seeded".
...
export async function findOrCreateContact(
  ...
  // Conflict-safe: a first-time member sending two quick messages spawns two
  // concurrent invocations that both miss the SELECT above. Without this, the
  // loser throws a unique violation and that member's first ever contact with
  // the church becomes an error message.
  ...
  if (!raced) {
    throw new Error(`contact vanished after insert conflict: church=${churchId} phone=${phone}`);
  }
...
/** Thrown when a menu exceeds WhatsApp's interactive-list row cap. Distinct from a
 *  Graph API failure so sendReply can fall back on this and only this. */
export class MenuTooLongError extends Error {}

/** Truncate to <=24 UTF-16 code units without splitting a grapheme. Plain .slice()
 *  severs emoji surrogate pairs -- and every menu label here starts with an emoji. */
export function truncateRowTitle(label: string): string { /* Intl.Segmenter, granularity: 'grapheme' */ }
...
export async function sendReply(
  ...
  // The catch is deliberately NARROW: only a too-long menu falls back. A Graph API
  // failure must propagate, or we would re-POST with the same credentials against the
  // same conditions -- burying the real error and risking a duplicate message if Meta
  // already queued the first one.
  try {
