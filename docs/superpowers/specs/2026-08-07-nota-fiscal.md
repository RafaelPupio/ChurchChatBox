...
**Design doc** · 2026-08-07 · Status: proposed · **Revision 2** (two independent verification lenses closed — see "Revisions")
...
The guard on both terminal-by-hand actions is therefore one clause: **`AND (status = 'erro' OR last_attempt_at IS NULL)`**, on top of the state list each already had. In words: *a human may end a row by hand only when no vendor call is outstanding against its current ref.* `erro` qualifies because it is a refusal we read; `bloqueada` and `pendente` qualify only when nothing was sent.
...
| **Refund → `dispensada`** | `AND status IN ('pendente','bloqueada','erro')` | Do nothing this pass; no watermark; re-evaluated next pass. |
...
|---|---|---|
| `emitida` | Attempt `cancelar(ref, motivo)`. `cancelada` → `cancelada`. `recusada` (**a refusal we read**) → **`cancelamento_falhou`**, top of the queue, copy that names the prazo and tells Rafael to call the accountant. `indisponivel` → **no terminal state**: write `erro_codigo = 'cancelamento_pendente'`, leave the row `emitida`, try again next pass. | `cancelada` and `recusada`: yes, with the outcome. `indisponivel`: **no** — nothing was decided. |
| `pendente` / `bloqueada` / **`erro`** | → `dispensada`, reason `estornada`. **Never issue a nota for a refund we can see.** | Yes, in the same statement. |
...
**The rest of the table was re-audited against that rule and the remaining arms hold.** `emitida` moves state (and, on `indisponivel`, deliberately does not consume the event). `dispensada` and `cancelada` are terminal with no button that issues from them. `cancelamento_falhou` is terminal and its document already exists. `manual` is terminal and issuance is over. The release-on-save statement cannot resurrect any of them: it is keyed on `status = 'bloqueada'`, and a refunded `bloqueada` row is now `dispensada` before it can be released. **`enviando` / `processando` are the only arms that consume nothing, which is correct and is why they alone are re-selected.**
...
4. **Sinalizadas** — rows carrying a flag in a non-error state (`estorno_parcial`, `estorno_manual`, `cancelamento_pendente`). They are not broken, but they are a decision or an unfinished vendor conversation waiting on Rafael, and the collapsed band would hide them.
...
**Consultar no emissor was listed as an action and never defined, and this revision defines it because the guard above leans on it.** It is `consultar(ref)` on demand, offered on **any row whose `last_attempt_at` is set** — not only the two states the settle pass sweeps — and its outcomes are applied by the same conditional writes the settle pass uses: `emitida` → `emitida` with the document recorded, `rejeitada` → `erro`, `processando` → `processando`, and `desconhecida` / `indisponivel` → **no write at all**. `consultar` is a read and may be called any number of times, which is why it needs no state guard of its own.
...
- **`CancelOutcome`, all three arms** — `cancelada` → `cancelada` with the watermark; `recusada` → `cancelamento_falhou` with the watermark; **`indisponivel` → the row stays `emitida`, `erro_codigo = 'cancelamento_pendente'`, no watermark, and the next pass calls `cancelar` again**, asserted on the stub. A two-arm implementation passes every test that omits the third, which is exactly how the timeout became a refusal.
...
- **Queue bands** — every status maps to exactly one band, asserted over the full enum so a new state cannot be added without a home: specifically `processando` and a fresh `enviando` appear in band 6 rather than nowhere, and `dispensada` appears in the collapsed band rather than being dropped. Plus: within bands 5 and 7 the rows come back ordered by `notaUrgency`, so a three-month-old blocked row with a future competência sorts **above** a fresh one with a nearer deadline.
