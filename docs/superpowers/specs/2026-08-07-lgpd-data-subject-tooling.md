...
**Design doc** · 2026-08-07 · Status: proposed · revision 5, after adversarial review, revision, re-attack, verification and an adversarial review of the derived implementation plan — see Revisions

## Overview

A member writes to a church's WhatsApp number and asks: *"o que vocês guardam sobre mim?"*, *"me manda uma cópia"*, *"apaga tudo"*. Today the church can answer the first question by scrolling the Caixa de Entrada, and cannot answer the other two at all — there is no export, no delete, and nothing that ever removes old data.

**When this spec was written, the 🔒 Privacidade item promised members *"as conversas são apagadas após 12 meses"* while nothing in the codebase made that true. That promise has since been withdrawn, and the sentence quoted here is no longer in the product.** On 2026-08-08, commit `4bc83aa` — *"stop promising a deletion the product cannot perform"* — replaced it with `*Por quanto tempo:* enquanto a igreja precisar deles para te atender. Você pode pedir a exclusão a qualquer momento.`, on the reasoning that an unkept promise sitting inside the one menu item whose whole job is telling members the truth about their data is worse under LGPD than a promise never made. The source comment left there points at this spec and asks whoever ships the purge to restore the 12-month wording **in the same commit**. So the gap this subsystem closes is no longer "the product lies"; it is that the product currently says the honest, weaker thing, and this subsystem is what earns the stronger one back. See "The 🔒 Privacidade text" for the two frozen bodies the rollout has to recognise. (Owner decision 6 still reads "matching what the 🔒 Privacidade item already tells members"; that premise is the withdrawn sentence, and the decision itself — purge at 12 months — is unaffected and not reopened here.)
...
|---|---|---|
| `contact` (`schema.ts:83-98`) | `phone` (the identifier — it *is* the person), `name` (whatever WhatsApp reports as their profile name) | The mere existence of a row associates a natural person with a church → **religious conviction, Art. 5 II sensitive** |
| `message` (`schema.ts:100-112`) | `body` — free text of everything said in both directions; `wa_message_id` | Ordinary personal data, unbounded content. Media is **not** stored: the webhook writes `null` for anything that is not text or a list reply (`webhook/route.ts:88-94`) |
| `prayer_request` (`schema.ts:114-121`) | `text` — free text, written in the most confessional state the product ever puts a member in | **The most sensitive row in the database.** Routinely carries health, family, financial and religious detail — three of Art. 5 II's categories in one column |
| `admin_user` (`schema.ts:124-147`) | Church staff `email`, `name`, `password_hash` | Staff, not members. Different data subject, different flow (see Out of scope) |
...
-- index (re-verified against main on 2026-08-11: `message` declares only
-- `waMessageIdUq`, schema.ts:110-112; `prayer_request` declares no third
-- argument at all, schema.ts:114-121). Each index below is
...
Drizzle addition to `src/db/schema.ts` — **one import line is added and none is edited.** The original draft claimed "nothing existing is modified", which was false; revision 1 corrected it to "both import lines are edited", which has since become false in the other direction. What is true on `main` today: the `drizzle-orm/pg-core` line already imports everything this subsystem needs, and `sql` is the only genuinely new name.

```ts
// lines 1-3, UNCHANGED — `index` is already imported (and so is `unique`, which
// this subsystem does not use):
//   import {
//     boolean, index, integer, pgEnum, pgTable, text, timestamp, unique, uniqueIndex, uuid,
//   } from 'drizzle-orm/pg-core';

// new import: schema.ts imports nothing from 'drizzle-orm' itself today, and the
// partial-index predicate needs sql``.
import { sql } from 'drizzle-orm';
```

> **Correction — the scheduled `index` edit is withdrawn, and why it drifted matters more than the edit.** Revision 1 wrote that "`schema.ts:1-3` imports `uniqueIndex` but not `index`" and scheduled the edit adding it. **That was true when it was written and is not true now.** Verified against git rather than recalled: at `b24efb1` (2026-08-07, the day this spec was written) line 2 read `boolean, integer, pgEnum, pgTable, text, timestamp, uniqueIndex, uuid`. `index` arrived on 2026-08-08 with `password_reset_token`'s `adminUserIdIdx` (`2a7db46`), and `unique` on 2026-08-10 with the `webhook_failure` table (`7bc4e1e`). Both postdate this spec, neither was added for it, and an implementer following revision 1 literally would have added a duplicate import name and failed the build on the first `tsc`.
>
> **The lesson is not about this line.** A spec that cites source by line number is citing a moving target: `main` moves, the spec does not, and a citation that was verified once decays into an assertion. **Every line citation in this document must be re-verified against source at implementation time, not trusted.** Revision 5 re-verified the `schema.ts` and `church-defaults.ts` families and found the whole `schema.ts:NN` family had shifted by +21 lines; the other files are listed there as re-verified, drifted, or unchecked, and the ones still marked drifted are the reader's warning that this decay is ongoing rather than closed.
...
`getChurchById` stays in the writable path only. Its existence check there is in any case near-redundant: `admin_user.church_id` is `ON DELETE CASCADE` (`schema.ts:126`), so a deleted church takes its admin rows with it and `findAdminById` already returns `undefined`. Its real job is supplying `status` and `graceUntil`, which only the suspension test needs. **No read page changes behaviour.**
...
- The response body is a `ReadableStream` **whose drain loop runs in `pull()`, not in `start()`**. The route writes the header object, then pages, then the footer, as UTF-8 chunks, and the next chunk is produced only when the consumer asks for one. **At most one page — 1 000 rows — is in memory at a time**, and that is true of the *response* only because of `pull()` — see the retraction below.
...
> **Correction — revision 4's memory bound was false, and is retracted rather than softened.** Revision 4 wrote, in these words: "**At most one page — 1 000 rows — is in memory at a time.**" That is true of the **database result set** and of nothing else. The route revision 4 actually described enqueues from `start()` without consulting `controller.desiredSize`, and `start()` runs to completion whether or not anyone is reading: the chunks pile up in the stream's own internal queue, so peak memory was bounded by the **50 000-row ceiling**, not by the page size — a bound fifty times looser than the number written beside it. This is the same defect class as the fabricated citation recorded above: a sentence that reads as verified and is not. The claim is not weakened here; the mechanism that makes it true is specified instead.

**How big "bounded by the ceiling" actually was, stated as far as it can be verified.** `message.body` is `text` with no length constraint (`src/db/schema.ts:108`), so the schema bounds a single chunk not at all. The repo's own note records 4 096 characters as WhatsApp's plain-text cap and 1 024 as the list-body cap (`src/lib/validation.ts:5-6`), and church-authored text is held to `CHURCH_TEXT_MAX = 1024` (`:7`). If the 4 096 figure holds for *inbound* bodies — which nothing in this repository can confirm, since there is no Meta app here — the worst case under a `start()`-enqueue design is on the order of 200 MB per collection, on precisely the member whose Art. 18 V deadline this route exists to serve. The order of magnitude is the point, not the number; it is listed in "What cannot be verified here" rather than asserted.

**The mechanism, because "one page" has to be earned.** Both export routes — `GET /api/dados/[contactId]` and `GET /api/dados/oracoes-expirando` — put the drain loop in `pull(controller)`:

- `start()` writes the header and nothing else. Every page after that is fetched inside `pull()`, **one page per invocation**, so the consumer's demand is what paces the database round-trips and the internal queue never grows past a page.
- The keyset cursor, the emitted-row counter, the budget start time and the "which collection am I in" marker live **outside** the loop, in the stream's closure, because `pull()` is re-entered rather than resumed. A `for(;;)` drain written *inside* `pull()` re-creates the original defect with an extra function around it, and would pass any test that only checks the output bytes.
- The footer (`aviso`, `continuacao`) is written by the `pull()` invocation that discovers the end, the ceiling or the budget, immediately before `controller.close()`. There is no separate flush step for a cancelled consumer to skip.
- The 45 s budget is measured from the first `pull()`, not from the request, so a slow consumer spends its own time rather than the function's.

**If the implementation or the plan shows the `start()` form, that is this defect and not a variant of it.** The derived implementation plan is being brought into line with this section; a reader who finds a drain loop enqueuing from `start()`, or a code comment claiming a one-page bound the loop beneath it does not deliver, is looking at the thing this correction names.
...
The current text (`src/lib/church-defaults.ts:38-63`; the body itself is `:42-62`) needs four changes and one removal:
...
**Rollout.** `PRIVACY_ITEM` is a *seed*: each church holds its own editable `menu_item` row (`provisioning.ts` copies it in at creation; `seedPrivacyItem` repairs it), so changing the constant updates nobody. `src/lib/church-defaults.ts` therefore keeps **every** previous seeded body frozen, as

```ts
export const PRIVACY_ITEM_PREVIOUS_BODIES: readonly string[] = [ /* v0, v1 */ ];
```

and the owner console gains **"Atualizar texto de Privacidade"**, which rewrites the row only when its current body is byte-identical to **one of** those. A church that edited its own text is never overwritten — the vendor may replace vendor-authored text, never the controller's own words. Churches whose text matches none of the frozen bodies are listed in the owner console so Rafael can call them. With one church live today this costs one click.

> **Correction — revision 4 specified `PRIVACY_ITEM_V1_BODY`, a singular constant, and that is a defect rather than a naming preference.** The paragraph already said "byte-identical to *a known previous default*", plural in its own sentence, while the constant it named could hold one body. There are demonstrably **two** previous seeded bodies, confirmed from git history rather than recalled:
>
> - **v0** — seeded 2026-08-07 with `provisionChurch` (`8f67fb3`). Its retention line promised `*Por quanto tempo:* as conversas são apagadas após 12 meses.`
> - **v1** — 2026-08-08, `4bc83aa`, *"stop promising a deletion the product cannot perform"*. The 12-month promise was withdrawn because nothing deleted anything yet, and replaced with `*Por quanto tempo:* enquanto a igreja precisar deles para te atender. Você pode pedir a exclusão a qualquer momento.` This is the live text today. No commit since has touched `PRIVACY_ITEM.bodyText` — verified by diffing `4bc83aa..HEAD` on that file.
>
> **What a single-body constant would cost.** The rollout rewrites a row only on a byte-identical match, so a church still carrying v0 would match nothing: never updated, never listed as edited, and therefore **invisible in both directions at once** — silently left on text that promises a 12-month deletion the product did not perform then, while every other church moves to v2. The one condition the console exists to detect is the one it could not see. Plural is not defensive coding; it is the difference between a rollout that terminates and one that leaves a church behind with no signal.
>
> **`4bc83aa` also corrected the one live church's row** — by hand, and only where the exact v0 sentence was still present. That was a manual operation outside the repository, so nothing here can verify which rows it reached; it is listed in "What cannot be verified here". The rollout must not assume it succeeded, which is another way of saying v0 stays in the frozen list.
>
> **The codebase already learned this once, on the neighbouring column.** `church.courtesy_text`'s own schema comment (`src/db/schema.ts:39-56`) records the same failure in miniature: the seeded wording was improved within the hour of the migration being generated, the constant and the backfilled rows diverged immediately, and the comment's conclusion is that closing such a gap "is a data migration, written deliberately — and only for rows a church has not edited". That is precisely this rollout, and the plural constant is what lets it be written.
>
> The implementation plan derived from this spec already uses `PRIVACY_ITEM_PREVIOUS_BODIES: readonly string[]` for exactly this reason.
...
- **Export backpressure is real, not claimed (B4)** — the one-page memory bound is a *behavioural* property and is tested as one, because a `for(;;)` drain inside `pull()` produces byte-identical output to a correct one and would pass every test above. With the pager mocked to count its invocations and 10 000 rows behind it, get the stream's reader, read **one** chunk, and assert the pager has been called at most twice — once for the header counts, once for the first page. Under the `start()` design this assertion fails with the pager exhausted, which is the point: the test distinguishes the two implementations, and no test that only inspects the output bytes can. A static assertion that the route defines `pull` and that `start` contains no paging loop is a cheap second fence, in the same style as the route-declaration test below.
...
- **The real upper bound on a single exported chunk.** `message.body` is `text` with no length constraint (`src/db/schema.ts:108`). The repo's own note records WhatsApp's caps as 4 096 characters for plain text and 1 024 for a list body (`src/lib/validation.ts:5-6`), but that note is about what this product *sends*; nothing here observes what Meta *delivers*. The ~200 MB worst case quoted under "Bounded, and actually streamed" is an order of magnitude derived from that note, not a measurement — which is exactly why the bound has to come from `pull()` rather than from an assumption about body length.
- **Which live `menu_item` rows commit `4bc83aa` actually corrected.** That commit's message says the one live church's Privacidade row was rewritten by hand, matching only the exact v0 sentence. The operation happened in the database, not in the repository, so this spec cannot confirm it reached every row or that no v0 row survives. The rollout is specified to recognise **both** previous bodies regardless.
...
**2026-08-11 — revision 5**, after an adversarial review of the **derived implementation plan** (`docs/superpowers/plans/2026-08-11-lgpd-data-subject-tooling.md`). This is the first pass whose findings came from *downstream* rather than from a review of the spec itself: writing the plan forced this document's claims through an implementer's hands, and three of them did not survive it. Nothing was re-architected, no settled owner decision was reopened, and every existing revision entry is left exactly as written.

**Blockers.**

- **B4 (the export's memory bound was false)** → **retracted in place and replaced by the mechanism that makes it true.** Revision 4 wrote "**At most one page — 1 000 rows — is in memory at a time.**" That holds for the *database result set* and for nothing else: the route it specified enqueues from `start()` without consulting `controller.desiredSize`, so `start()` runs to completion whether or not anyone is reading and the chunks accumulate in the stream's internal queue. Peak memory was therefore bounded by the **50 000-row ceiling** times a `message.body` the schema does not length-constrain (`schema.ts:108`) — fifty times looser than the number written beside it, on precisely the member whose Art. 18 V deadline the route exists to serve. Closed by moving the drain loop into **`pull()`** on both export routes, with the cursor, the row counter and the budget clock held in the stream's closure because `pull()` is re-entered rather than resumed, and the footer written by the invocation that discovers the end. The sentence was not softened into something vaguer and still unbacked; the claim now names a mechanism that delivers it, and the Testing section gains **Export backpressure is real, not claimed** — a behavioural assertion, because a `for(;;)` drain inside `pull()` emits byte-identical output and would pass every other export test. Making the claim testable is the same remedy revision 3 applied to "a test says so" under N8. The order-of-magnitude figure for the old bound is listed as unverifiable rather than asserted, since nothing here observes what Meta delivers as a message body. The same claim also stood in the Decisions table and is corrected there; it stands uncorrected in **revision 1's I2 bullet**, which is the record of what revision 1 said and is left alone by this document's convention.
- **B5 (a scheduled edit to existing source was already done, by someone else, for something else)** → **withdrawn, with the reason it drifted written next to it.** Revision 1 wrote that `schema.ts:1-3` "imports `uniqueIndex` but not `index`" and scheduled the edit adding `index`. Verified against git: **the claim was true on the day it was written** — at `b24efb1` (2026-08-07) line 2 read `boolean, integer, pgEnum, pgTable, text, timestamp, uniqueIndex, uuid`. `index` arrived on 2026-08-08 with `password_reset_token`'s `adminUserIdIdx` (`2a7db46`); `unique` arrived on 2026-08-10 with the `webhook_failure` table (`7bc4e1e`). Neither was added for this subsystem, and an implementer following revision 1 literally would have written a duplicate import name. **`import { sql } from 'drizzle-orm'` is the only genuinely new import**, and the section now says so. The generalisation is stated in the section rather than left as a one-line fix: a line citation is a claim about a moving target, and every one in this document must be re-verified at implementation time.
- **B6 (`PRIVACY_ITEM_V1_BODY` is singular and the rollout needs plural)** → **corrected to `PRIVACY_ITEM_PREVIOUS_BODIES: readonly string[]`**, matching what the plan already uses. Confirmed from git history, not recalled: there are **two** previous seeded bodies — **v0** (`8f67fb3`, 2026-08-07), which promised `as conversas são apagadas após 12 meses`, and **v1** (`4bc83aa`, 2026-08-08, *"stop promising a deletion the product cannot perform"*), which withdrew that promise and is the live text today. `4bc83aa..HEAD` shows no further change to `PRIVACY_ITEM.bodyText`. This is a **defect, not a naming preference**: the rollout rewrites a row only on a byte-identical match, so a church still carrying v0 would match nothing — never updated, never listed as edited, invisible in both directions at once, left on text promising a deletion the product did not perform. The one condition the console exists to detect is the one a single-body constant could not see. `4bc83aa` also corrected the live church's row by hand, in the database rather than in the repository, so that is recorded as unverifiable rather than relied on.

**Also corrected while re-verifying:** the Overview's opening claim that the 🔒 Privacidade item "already promises members that *as conversas são apagadas após 12 meses*" is **no longer true of the product** — that sentence was removed on 2026-08-08 and the Overview now records the withdrawal, the replacement wording, and the source comment asking that the 12-month promise be restored in the same commit that ships the purge. Owner decision 6 rests on the same withdrawn premise; the decision itself (purge at 12 months) is unaffected and is not reopened.

**Line citations re-verified against `main` this pass.** The whole `schema.ts:NN` family had shifted by **+21 lines** — 18 from the `church` table's new `courtesy_text` column and its comment, 3 from a new import and comment at the top of the file — and is corrected: `contact` `:62-77`→`:83-98`, `message` `:79-91`→`:100-112`, `prayer_request` `:93-100`→`:114-121`, `admin_user` `:103-112`→`:124-147`, and the four single-line cites `:74`→`:95`, `:82`→`:103`, `:96`→`:117`, `:105`→`:126`. `src/lib/church-defaults.ts:31-45`→`:38-63`. The in-migration note that `message` declares only `waMessageIdUq` and `prayer_request` declares no index callback **re-verified as still true** (`schema.ts:110-112`, `:114-121`).

**Confirmed unchanged, so the reader knows what was checked and passed:** `tests/privilege-boundary.test.ts` `:24-28`, `:30-33`, `:40-48`, `:45`, `:58`, `:77`, `:82-96`, `:121-135`; `src/lib/church-status.ts:13-21`; `src/lib/repo/contact.ts` `:20-28`, `:33`, `:55`, `:72,79,89`; `src/lib/repo/message.ts:21`/`:22`; `src/lib/repo/platform.ts:3`; `src/lib/repo/prayer-admin.ts:5-12`; `src/app/owner/(protected)/page.tsx:3`; `src/app/owner/(protected)/[churchId]/page.tsx:4` and `:35`; `src/app/owner/(protected)/[churchId]/actions.ts:56-83`; `tests/tenant-isolation.test.ts:166-175`; `src/app/api/blob/upload/route.ts:9-25`; `.superpowers/sdd/owner-decisions-2026-08-07.md:58-77`.

**Verified as drifted and deliberately NOT rewritten this pass**, because the review that prompted revision 5 scoped three corrections and a citation sweep is not one of them. They are recorded here so the next reader treats the numbers in the body as suspect rather than as verified, and so nobody re-does the checking:

| Cited in the body | What it claims | Where it actually is on `main`, 2026-08-11 |
|---|---|---|
| `writable.ts:26-27` | `verifyWritable` calls `getChurchById` | `:55` |
| `writable.ts:29-31` | `requireWritableSession` blocks suspended churches | the test is in `verifyWritable`, `:58-60` (doc line `:44`) |
| `writable.ts:75-84` (three places) | `requireReadableSession` performs no status check | `requireReadableSession` is `:105-122`; the "deliberately does NOT block a suspended church" comment is `:96-97`. `:75-84` is now `checkWritableSession` |
| `writable.ts:86-92` | `UNAUTHENTICATED_MESSAGE` / `REVOKED_MESSAGE` | `:124-130` |
| `platform.ts:6-8` | the OWNER-ONLY comment | `:7-9` |
| `platform.ts:22` (three places) | `listChurches`'s argument-less `db.select().from(church)` | `:38-39` |
| `platform.ts:1-4` | "imports only `drizzle-orm`, `@/db/client`, `@/db/schema`, `@/lib/church-status`" | imports are `:1-5` and now also include `webhookFailure` and `@/lib/migration-drift`. **The conclusion still holds** — none of them is restricted — but the enumeration is stale, and it is the enumeration the boundary argument leans on |
| `whatsapp.ts:24-26` | the image-caption payload, anchoring the 1 024-character cap | `buildImagePayload` is `:29-31`. The constant `CHURCH_TEXT_MAX = 1024` is `src/lib/validation.ts:7`; **no 1 024 appears in `whatsapp.ts` at all**, and validation.ts's comment is about the list-body cap, not an image caption |
| `whatsapp.ts:84-98` | `verifySignature`'s constant-time compare | `verifySignature` is `:77-89`; the compare is `:83-88` |
| `whatsapp.ts:120-123` | the `Graph API ${status}: ${detail}` throw | `:109-112` (the throw is `:111`), inside `post()` |
| `whatsapp/webhook/route.ts` `:62`, `:80`, `:85`, `:88-94`, `:98`, `:180` | the whole webhook family | uniformly off by one: `:63`, `:81`, `:86`, `:90-95`, `:99`, `:181` |
| `caixa/actions.ts:43` | the catch-all `console.error` | `:51`, and the logged string is `'Reply send failed'`, not `'Reply failed'`. `:43` is now the `getChurchById` call. The plan's own review found this independently |
| `caixa/actions.ts:28-33` | a reply needs a conversation row and an open window | `:36-41` |
| `src/lib/repo/admin.ts:33-35` | "staff removal already hard-deletes the row" | `admin.ts` contains no `removeStaff`. The hard delete is `deleteAdmin` at `:66`; the Server Action `removeStaff` is `src/app/admin/(protected)/configuracoes/actions.ts:140` |
| `blob/upload/route.ts:55-56` | "the URL is persisted on `menu_item.image_url`" | **the route persists nothing** — its `onUploadCompleted` is empty (`:71`). The persistence is `src/app/admin/(protected)/conteudo/item-actions.ts` (`:28`, `:54`). The Blob-holds-nothing conclusion is unaffected; the citation points at the wrong file |
| `inbox.ts:54` | an `update(contact)` site | `inbox.ts`'s only `update(contact)` is `:95`; `:54` is inside a doc comment |
| `contact.ts:49-54` described as "a seven-line comment" | — | the comment is **six** lines |

**Not verifiable here, and not asserted as if it were:** whether Meta delivers message bodies under the 4 096-character cap this repo records for its own sends, which is what turns the old `start()` bound from an order of magnitude into a number; and which live `menu_item` rows commit `4bc83aa` actually rewrote by hand. Both are in "What cannot be verified here". Also unchecked this pass: every citation to a file this subsystem *creates* rather than reads, since there is nothing yet to check them against.
